...
#7: HTML (guerzhoy)
#8: looking for search results (guerzhoy)
def num_search_results(query):
    import urllib.request
    url = "https://ca.search.yahoo.com/search?p=" + query #just the format of search results
    f=urllib.request.urlopen(url) #readscontents of url
    page = f.read().decode("utf-8")
    f.close()
    #print(page) #prints the page
    ind = page.find("search results</span>") #location of search results
    #the number of search is right before this, so need to find the stuff that coems before hte number too to find the number index
    s=page[ind-100:ind]#the approximate area
    about_loc = s.find(">About")
    return int((s[about_loc+len(">About")+1:-1]).replace(",", "")) #prints the number, takes out the comma, turns into integer
def compare_nums(query1, query2):
    n1=num_search_results(query1)
    n2=num_search_results(query2)
    if n1>n2:
        winner, loser = query1, query2
    else:
        winner, loser=query2, query1
    print(f"Winner: {winner}, Loser: {loser}")

compare_nums("EngSci", "TrackOne")
